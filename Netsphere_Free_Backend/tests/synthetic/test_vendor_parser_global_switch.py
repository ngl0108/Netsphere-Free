...
def test_vendor_parser_global_switch_group_passes():
    cases = [c for c in load_vendor_fixture_cases(DEFAULT_VENDOR_FIXTURES_ROOT) if c.fixture_group == "global_switch"]
    assert len(cases) >= 10
    ...
    assert int((by_type.get("inventory") or {}).get("total") or 0) >= 5
    assert int((by_type.get("neighbors") or {}).get("total") or 0) >= 5
